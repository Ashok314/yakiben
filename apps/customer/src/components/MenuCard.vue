<template>
  <div @click="router.push({ name: 'itemDetail', params: { id: item.id } })"
    ...
    <div class="relative aspect-video transition-all duration-300 bg-gray-100">
      ...
      <div class="absolute bottom-2 right-2 flex gap-2 items-center z-20" @click.stop>
        ...
        <div v-if="quantity > 0"
          ...
          <button @click.stop="handleRemoveOne"
        ...
        <!-- Add Button (if 0) -->
        <button v-else-if="item.available" @click.stop="handleAddToCart" class="shadow-lg rounded-full p-2 flex items-center justify-center transition-all duration-300
                 bg-white text-primary hover:bg-primary hover:text-white border border-gray-100">
          <span v-if="hasRequiredCustomizations && !canUseDefaults" class="text-xs font-bold px-2">詳細</span>
    ...
    <div class="p-3 pt-2">
      <div class="space-y-1">
        <div class="flex justify-between items-start">
          <h3 class="font-bold text-gray-900 text-sm leading-tight line-clamp-1">{{ item.name }}</h3>
        ...
        <div class="flex justify-between items-end">
          ...
          <span v-if="item.customizations && item.customizations.length > 0"
...
const quantity = computed(() =>
  getItemQuantity(props.item.id, currentDefaultCustomizations.value)
);
